...
#define REMINDER_LINE "OmNiTTY-R v" OMNITTY_VERSION \
                      ...
                      "  \005F7\007:mcast \001F9\007:maximize  \002F10\007:add-list"
...
#define MENU_ITEM_NUM		128
#define MENU_ITEM_IP_LENGTH	128
#define MENU_ITEM_IP_NUM	1024

#define GET_ARR_LEN(arr)	(sizeof(arr) / sizeof(arr[0]))
#define GET_ARR_IPLIST_LEN(arr)	(sizeof(arr) / sizeof(arr[0]))
...
struct menu_struct { 
		...
		char iplist[MENU_ITEM_IP_NUM][MENU_ITEM_IP_LENGTH];
} menus[MENU_ITEM_NUM];
...
void wins_init(int *vtrows, int *vtcols, int list_win_chars, 
                                                int term_win_chars,int tag) {
   //int termcols, termrows, A, B, C;
   int termcols, termrows, A, B;
   const char *p;
   ...
   *vtrows = termrows - 3;
   ...
   //vtwin   = newwin(termrows-3, *vtcols, 1, 0);
   if(tag){
	   vtwin   = newwin(termrows-3, *vtcols, 1, C);
   }else{
	   vtwin   = newwin(termrows-3, 0, 1, 0);
   }
...
void menu_list(void) {
	...
	int termwidth, termheight, choice, old_choice;
	int i; int ch; int n;
	WINDOW *w;

	getmaxyx(stdscr, termheight, termwidth);
	//w = newwin(MENU_LINES, MENU_COLS, (termheight - MENU_LINES)/2, (termwidth  - MENU_COLS )/2);
	w = newwin(menucounts+5, MENU_COLS, 10, menucounts);
	//w = newwin(100, MENU_COLS, 0, menucounts);
	werase(w);
	...
	wmove(w, i = 1, 1); 
	mvwprintw(w,STARTY, STARTX, menus[0].name);
	//mvwprintw(w,STARTY, STARTX, "Please choose IP-List");
	//for (i = 1; i < GET_ARR_LEN(items); ++i) {
	for (i = 1; i < sizeof(menus); ++i) {
		//for (i = 1; i < sizeof(items); ++i) {
		if (1 == i) wattron(w,A_REVERSE);
		mvwprintw(w,STARTY+i, STARTX+2, "%s", menus[i].name);
		//mvwaddch(w,STARTY+i, STARTX+2, menus[i].name);
		//wmove(w, ++i, 1);
		//waddch(w, menus[i].name);
		//mvwaddch(w,i,STARTX+2,menus[i].name);
		if(1 == i) wattroff(w,A_REVERSE);
	}
	wmove(w, ++i, 1);
	...
	if(!menucounts) menucounts = 1;
	choice = old_choice = 1;

	for ( ; ; ) {
		ch = getch();
		switch (ch) {
			case KEY_UP: choice = choice == 1 ? menucounts : choice - 1;		break;
			case KEY_DOWN: choice = choice == menucounts ? 1 : choice + 1;	break;
			case ESC:	goto DELWIN;
			case ENTER:	goto ADDMAC;
			default:	break;
		}
		mvwprintw(w,STARTY + old_choice, STARTX + 2, menus[old_choice].name);
		wattron(w,A_REVERSE);
		mvwprintw(w,STARTY + choice, STARTX + 2, menus[choice].name);
...
ADDMAC:
	for( n=0;n < GET_ARR_IPLIST_LEN(menus[choice].iplist);++n)
		machmgr_add(menus[choice].iplist[n]);
DELWIN:
	delwin(w);
}
...
int read_rc(){
	int i=0, j=1;
	FILE *fp ;
	char fbuf[MENU_ITEM_IP_LENGTH],filename[128];
	char sbuf[MENU_ITEM_IP_LENGTH + 3];
	struct passwd *user;

	user = getpwuid(getuid());
	sprintf(filename,"%s/"OMNITTYRC,user->pw_dir);

	fp = fopen(filename,"r");

	if(fp==NULL) {
		memcpy(menus[0].name,"file ~/.comnittyrc Can't found!!!",50);
		return 1;	
	}

	memcpy(menus[0].name,"Please choose IP-List",24);

	while(fgets(fbuf, MENU_ITEM_IP_LENGTH, fp) != NULL){ 
		char *be, *en;
		if((be=strstr(fbuf,"[")) && (en=strstr(fbuf,"]"))){
			i++;
			sprintf(sbuf,"%d %s",i,fbuf);
			memcpy(menus[i].name,sbuf,strlen(sbuf)-1);
			//memset(menus[i].iplist,0,sizeof(menus[i].iplist));
			j=0;
		}else{
			if(!i) continue; //[]can't found!
			if(strlen(fbuf) > 2){
				if(!strchr(fbuf,'#'))
					memcpy(menus[i].iplist[j++],fbuf,strlen(fbuf));
			}
		}
	}

	menucounts = i;
	if(!i) memcpy(menus[0].name,"Please configure \"~/.comnittyrc\"",50);
	fclose(fp);
	return 0;
}

void add_machine_list() {
	read_rc();
	menu_list();
}
...
int main(int argc, char **argv) {
   ...
   curses_init();
   wins_init(&vtrows, &vtcols, list_win_chars, term_win_chars,1);
   menu_init(minibuf);
   machmgr_init(listwin, vtrows, vtcols,1);

   void  machmgr_toggle_maximize(){
	   if(M = !M){
   		   int max_term_win_chars = getmaxx(stdscr);
		   wins_init(&vtrows, &vtcols, list_win_chars, max_term_win_chars,0);
		   curutil_attrset_blink(stdscr, 0xF9); 
	   }else{
		   wins_init(&vtrows, &vtcols, list_win_chars, term_win_chars,1);
		   machmgr_init(listwin, vtrows, vtcols,0);
		   curutil_attrset(stdscr, 0x40); 
	   }
	   wmove(stdscr, 0, 2);
	   waddstr(stdscr, "[Machines]");
   ...
   while (!quit) {
      ...
      switch (ch) {
         ...
         case KEY_F(10): add_machine_list(); break;
         case KEY_F(9): machmgr_toggle_maximize(); break;
